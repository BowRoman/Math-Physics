...
// gerenate Lagrange Polynomial interpolation between 3 control points
...
/// lagrange polynomial stuff
struct endPoint
{
	myVector cntrlPnt;
	float timeTag;
	endPoint(myVector &pt, float t = 0.f) : cntrlPnt(pt), timeTag(t){}
	void setTime(float t) { timeTag = t; }
};
...
struct LagrangePoly{
	...
	myVector currPoint;
	float averageSpeed;
	std::map<float, myVector> pathMap;
	std::map<float, myVector>::const_iterator currIt;
	bool isSet;

	LagrangePoly() :averageSpeed(0.0f), isSet(false) {};

	void set(myVector &p1, myVector &p2, myVector &p3, float timerange)
	{
		float dist1 = p2.dist(p1);
		float dist2 = p3.dist(p2);

		p[0] = new endPoint(p1, 0.0f);
		p[1] = new endPoint(p2, dist1 / (dist1 + dist2) * timerange);
		p[2] = new endPoint(p3, timerange);
		currPoint = p1;
		buildPathMap(timerange, 1.0/30.);
		isSet = true;
	}
	void buildPathMap(float timerange, float inc)
	{
		size_t totalCount = timerange / inc;
		float pathLen = 0.0;
		...
		{
			currentTime += inc;
			myVector pos = computeLagrange(currentTime);
	...
	void displayPath()
	{
		...
		glBegin(GL_LINE_STRIP);
			...
			{
				glVertex3f(mapItem.second.x, mapItem.second.y, mapItem.second.z);
			}
		glEnd();
		glPopMatrix();
	...
	myVector next(float tInc)
	{
		float pathAdvancement = averageSpeed * tInc; // find how much we should advance on the path.
		float nextPathLength = (*currIt).first + pathAdvancement;
	/*	std::cout << "next: pathAdvancement=" << pathAdvancement << ", currIt={" << (*currIt).first << ", (" << (*currIt).second.x << "," << (*currIt).second.y << "--";
		float diff = pathAdvancement;
		auto testIt = currIt;
		do 
		{
			diff = fabs(nextPathLength - testIt->first);
			testIt++;
		} while (testIt != pathMap.cend() && fabs(nextPathLength - (*testIt).first) < diff);
		*/
		auto testIt = pathMap.upper_bound(currIt->first);
		currIt = testIt;
		...
		std::cout << "---> (" << (*currIt).second.x << ", " << (*currIt).second.y << ")" << std::endl;
		return currIt->second;

	//	myVector tmpNext = computeLagrange(t + tInc);
	//	myVector tangent = tmpNext - currentPoint;
	//	tangent.normalize();
	//	myVector next = currentPoint + tangent * (speed *tInc);
	}

	myVector computeLagrange(float t)
	{
		...
		return  p[0]->cntrlPnt * mult1 + p[1]->cntrlPnt * mult2 + p[2]->cntrlPnt * mult3;
	}
} mLagrangePoly;


myVector lerp(myVector &start, myVector &end, float t, float timespan) // timespan = t_2- t_1
{
...
int Menu()
{
	...
	while(r!=eStart && r!= eStop)
	{
		...
		char *message = "Use L, H, G, B, Q, C to cycle among interpolation schemes.\n";
		...
		sprintf(interpolstyle, "interpolation method is %s %s!\n",( eInterpolation==Linear ? "Linear" : 
																 ...
																 eInterpolation == Hermite ? "Hermite" : 
																 eInterpolation == Lagrange ? "Lagrange" : "CatMulRom"), 
																 isSpline ? "spline" : "");
...
void updatePath(double timeInc)
{
	switch (eInterpolation)
	{
		...
		case Lagrange:
		{
			if (mLagrangePoly.isSet)
				currentPoint = mLagrangePoly.next(timeInc);
			break;
		}
		default:
...
void recalculateTimeStamps()
{
	...
	if (eInterpolation == Lagrange && controlPointCnt == 3)
		mLagrangePoly.set(contrlPnts[0].center, contrlPnts[1].center, contrlPnts[2].center, timeRange);
}
